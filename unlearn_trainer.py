...
class CustomFamilyTrainerForgetting(Trainer):
    ...
    def reliable_save_model(
        self, curr_save_dir, max_retries: int = 10, sleep_time: int = 5
    ):
        """
        ...
        os.makedirs(
            curr_save_dir, exist_ok=True
        )  # curr_save_dir = папка чекпоинта конкретного

        for attempt in range(1, max_retries + 1):
            print(
                f"[ReliableSave] Attempt {attempt}/{max_retries} -> Saving model to {curr_save_dir}..."
            )
            ...
            time.sleep(
                sleep_time
            )  # приостановка текущего потока программы на sleep_time секунд (чтобы FUSE-драйвер google drive закончил синхронизацию)
            ...
            if not missing_files:
                ...
                print(
                    f"[ReliableSave] Missing files after attempt {attempt}:{missing_files}"
                )
    ...
    def e_prepare_deepspeed(self, model):
        ...
        config_kwargs["optimizer"] = {
            "type": None
        }  # отключение оптимизатора, т.к. deepspeed вроде только на этапе инференса применяется
